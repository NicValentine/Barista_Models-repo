...
# ASSIGNMENT 13 - Partial Matching to Model
...
#    spreading activation commented out to better exemplify partial matching system    
#    dm_spread=DMSpreading(DM,focus)                  
#    dm_spread.strength=2                             
#    dm_spread.weight[focus]=1  

    # add partial matching to the model. 
    # PhD students have a high partial match

    partial=Partial(DM,strength=1.0,limit=-1.0) 
    partial.similarity('nico','spencer', 0.5)   
    partial.similarity('nico','brendan', 0.5)
    partial.similarity('spencer','brendan', 0.9) 
    # Spencer and Brendan have a high partial match.
    # So when the barista tries to rememeber spencers order of 
    # biscotti, it will at times misremember with an error of comission 
    # and serve spencer savoiardi lady fingers which are brendans order.


    DM.add('customer:nico side:biscotti')
    DM.add('customer:spencer side:biscotti') 
    DM.add('customer:rob side:cannoli')
    DM.add('customer:jim side:cannoli')
    DM.add('customer:david side:savoiardi_lady_fingers') 
    DM.add('customer:brendan side:savoiardi_lady_fingers') 
    ...
    def production_serve_drink(focus='task:espresso operation:serve_drink'):
        ...
        focus.set('task:side operation:side_biscut_recall customer:spencer')
    
        # 22) Recall side biscut of customer spencer
    def production_recall_side(focus='task:side operation:side_biscut_recall customer:spencer'):
        print('They also ordered a side with their espresso, what was it?')
        DM.request('customer:spencer side:?side',require_new=True) 
