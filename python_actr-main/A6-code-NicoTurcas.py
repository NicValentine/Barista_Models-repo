...
# Assignment 6 : Add not function to dm request

# We have taken the former barsita model and added declarative memory to it. 
# but in this model we exemplify how the ! (not) operator works. 
...
class MyAgent(ACTR):
    ...
    def production_make_latte_mix_steamed_milk_and_espresso(focus='task:latte operation:make_latte'):
        ...
        # Garnish request with not operator, which will cause the agent to always recall pumpkin_spice as the garnish
    def production_recall_garnish(focus='task:latte operation:recall_garnish'):
        print('Recalling garnish....')
        DM.request('garnish:?x!cinnamon') 
